...
void ccode::set_generator(unsigned int uint_gen,unsigned int uint_gen_id)
{
    this->uint_gen[uint_gen_id] = OctToDec(uint_gen);
}

void ccode::out_outputs() {
...
unsigned int ccode::next_state(unsigned int uint_state, bool b_input)
{
...
unsigned int ccode::next_state(bool b_input)
{
...
unsigned int ccode::next_output(unsigned int uint_state, bool b_input)
{
...
unsigned int ccode::next_output(bool b_input)
{
    uint_last_state = uint_current_state;
    unsigned int uintTmp = 0;
    if (!b_input)
...
unsigned int ccode::prev_state(unsigned int uint_state,bool b_input)
{
    //TODO retuen the right value
    return 0;
}

std::vector <unsigned int> ccode::prev_states(unsigned int uint_state)
{
    return vec_uint_prev_states[uint_state];
}
unsigned int ccode::prev_internal_state()
{
...
matrix <char> ccode::encode(const matrix <char> &mat_arg)
{
...
unsigned int ccode::OctToDec(unsigned int uintMask)
{
...
bool ccode::EvenParity(unsigned int uint_register)
{
    bool boolTmp = false;
    for (int i=0; i < 31; i++)
    {
...
std::string ccode::UIntToBinChar(unsigned int uint_register) {
